...
//        console.log('j: seralized track ' + result) 
        return result
    }
...
function Track(tracks, player, topviewplayer, color, position, autotrack, forwardtracker, bidirectionaltracker)
{
    ...
    this.drawtopmarker = function(frame, position)
    {
        ...
        console.log('position generated? ' + position.generated)
        if (position.generated && !position.occluded)
        {
//j: drawing bxes
//            console.log('drawtopmarker')
...
//        }
    ...
    //j: record tracked data
    this.recordtrackdata = function(data, start, stop) {
        console.log('record data called ' + data)
        if (!data) return;
        
        console.log(data)
        var path = data.boxes;
        //j: var i chagned 1 to 0, because server's response doesn't include start frame
...
function AutoTrackManager(tracks, track, forwardtracker, bidirectionaltracker)
{
    ...
    // variable to make sure there is only 1 request on the fly
...
//j: old code above. seems to wrongly add 1
        ...
        // Remove from list of request intervals
    ...
    this.makerequest = function(interval) {
        ...
        if (interval["canceled"]) {
            ...
            //         return;
            //     }
            //     console.log("TRACKING: interval end is null. using forward tracking")
            //     api = "trackforward";
            ...
            //     console.log("TRACKING: interval end is null. using bidirectional")
            //     args = [this.track.player.job.jobid, interval["start"],
            //         interval["end"], this.bidirectionaltracker, this.track.id];
            // }

            // j: only use forward tracking for now
            // Use the online trackers
            if (this.forwardtracker == null) {
                this.requestcomplete(interval, null);
                return;
            }
            console.log("TRACKING: using forward tracking")
            api = "trackforward";
            args = [this.track.player.job.jobid, interval["start"], interval["end"],
                    this.forwardtracker, this.track.id];

            // user haven't select a class, track.id is 0
            ...
            if (!this.waitingforresponse){
                ...
                this.requestcomplete(interval, null);                
                console.log("TRACKING: Another tracking in fly. abort current request")
            }
    ...
    this.schedulerequest = function(interval) {
        setTimeout(function() {
            console.log("TRACKING: Scheduling request in [" + 
                interval["start"] + ", " + 
                interval["end"] + "]"
            );
    ...
    {
        console.log("j: add end as key frame. frame: "+this.track.journal.rightmostframe())
    ...
    this.addcurrentkeyframe = function(callback)
    {
        ...
        console.log("j: add current key frame: " + frame)
        this.track.recordposition();
        this.addkeyframe(frame, callback);
    }

    // this.interval records all the frames user handlabled
    // Use a new key frame for tracking
    this.addkeyframe = function(frame, callback)
    {
        // check if user has already give bx a class
        if (this.track.id ==null){
            console.log("no track id. auto tracking not issued");            
            return
        }
        ...
        for (var i in this.intervals) {
            if (this.intervals[i]["start"] < frame && 
                ...
                // Issue the request
                console.log("j:issue request from addkeyframe")
                ...
                console.log("j: key frame in intervals boundries");
                added = true;
                var previnterval = this.intervals[i];
                previnterval["time"] = new Date();
                previnterval["callback"] = callback;
        ...
        //j: use 1 request, since we only support forward tracking now
        if (!added) {
            ...
            console.log("j: key frame not falling in any intervals.");
...
function Journal(start, blowradius)
{
    ...
    this.remove = function(){
        //j: temporary fix. need to remove all ui related to objectui.js or ui.js
        $("#manuallabelframes").empty();        
    }
    ...
    {
        // console.log("Marking " + frame + " position: " + position.xtl + " "+position.ytl + " "
        //              + position.xbr + " " + position.ybr + " generated? " + position.generated);
        ...
        if (!position.generated){
            //j: TODO: called too many time! for each mark, this is refreshed!!
            //j: render a series of button for hard labeled frames
            var hardlabels = [];
            for(var frame in this.annotations) {
                if (!this.annotations[frame].generated){
                    hardlabels.push(frame);
                }
            }
            //sort by values
            hardlabels=hardlabels.sort(
                function(a,b) {
                    if (isNaN(a) || isNaN(b)) {
                        return a > b ? 1 : -1;
                    }
            ...
            $("#manuallabelframes").empty();
            $('<p>' + hardlabels.length + ' Manually Labeled Frame</p>').appendTo('#manuallabelframes');
            for (var i in hardlabels){
                var frame = hardlabels[i];
                if (frame != null){
                    $("<button>").appendTo("#manuallabelframes").attr("id",frame).text(frame)
                    $("#"+frame).click(function(){
                        var frameID = this.id;
                        console.log('clicked hardlabel ' + frameID);
                        //j: need to trigger slide event
                        $("#playerslider").slider('value',frameID);
                        $("#playerslider").trigger('setCurrentFrame', frameID);
                        //j: set slider to current frame and trigger slide
                    });
                    hardlabels.push(frame);
                }
            }
    ...
    this.clearbetweenframes = function(frame1, frame2) {
        console.log("TRACKING: clear between frames (excluding both ends) : from " + frame1 + " to " + frame2);
        clearframes = []
        for (t in this.annotations) {
            time = parseInt(t);
    ...
    this.serialize = function()
    {
        ...
        //j: comment out using artificialrightframe
