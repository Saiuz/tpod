...
function TrackObject(job, player, activecontainer, donecontainer, color, copypastehandler, defaultclass)
{
    ...
    this.stateclassify = function()
    {
        ...
        if (length == 1)
        {
            this.finalize(firsti, false, false);
            this.statefolddown();
        }
        else if (this.defaultclass)
        {
            this.finalize(this.defaultclass, false, false);
            this.statefolddown();
        }
        else
        {
    ...
    this.finalize = function(labelid, done, isinit)
    {
        console.log("j: objectui finalize")
        this.label = labelid;
        ...
        // add keyframe only when user just draw this frame
        // do not rerun tracking when an object is first loaded
        if ( (!isinit) && this.track.autotrack) this.track.autotrackmanager.addcurrentkeyframe()        
        //j: disable initial tracking when done...  not sure what is the original intention here
